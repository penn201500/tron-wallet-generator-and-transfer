...
const bs58check = require("bs58check") // Using bs58check for Base58Check encoding
...
function privateKeyToTronAddress(privateKey) {
  ...
  const addressHash = keccak256(Buffer.from(publicKey, "hex")).slice(-20) // Get the last 20 bytes
  const addressHex = Buffer.from(addressHash).toString("hex") // Convert to hexadecimal string
  const tronAddressHex = Buffer.from("41" + addressHex, "hex") // Add Tron address prefix '41'
  const tronAddress = bs58check.encode(tronAddressHex) // Base58Check encode
